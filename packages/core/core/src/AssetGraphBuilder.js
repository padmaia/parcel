...
export default class AssetGraphBuilder extends EventEmitter {
  ...
  async init({
    ...
    console.log('ASSETGRAPHBUILDER init', entries, assetRequests);
  ...
  |}> {
    console.log('HAS INCOMPLETE NODES', this.assetGraph.hasIncompleteNodes());
  ...
  processIncompleteAssetGraphNode(node: AssetGraphNode, signal: AbortSignal) {
    console.log('PROCESSING INCOMPLETE GRAPH NODE', node);
    switch (node.type) {
  ...
  async runEntryRequest(request, signal) {
    console.log('RUNNING ENTRY REQUEST');
    let result = await this.requestTracker.runRequest(
      'entry_request',
      request,
      {signal}
    );
    console.log('ENTRY REQUEST COMPLETE');
